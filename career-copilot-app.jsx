 ...
 *
 * Persistence: window.storage, personal (not shared), three keys —
 * career-profile, career-applications, career-edit-signals.
 */
...
- Keep it concise so the whole response fits in a short budget: summary max 2 sentences; at most 3 work experience entries, max 3 bullets each, each bullet under 20 words; at most 2 projects, max 2 bullets each; cover letter under 180 words in 3 short paragraphs.
- Respond with ONLY one valid JSON object. No markdown fences, no commentary, no text outside the JSON.
${styleContext ? "\n" + styleContext + "\n" : ""}
JSON shape:
{"resume":{"summary":"","skills":[""],"experience":[{"company":"","position":"","dates":"","bullets":[""]}],"projects":[{"title":"","bullets":[""]}],"education":[{"school":"","degree":"","year":""}],"certifications":[{"name":"","provider":"","date":""}]},"coverLetter":""}`;
}
...
async function generateApplication(profile, jobDescription, preferences, editSignals) {
  ...
  if (!response.ok) throw new Error("The generator is unavailable right now (status " + response.status + ").");
...
${JSON.stringify(payload)}`;
  ...
  if (!response.ok) throw new Error("The copilot is unavailable right now (status " + response.status + ").");
  const data = await response.json();
  const text = (data.content || []).map((block) => (block.type === "text" ? block.text : "")).join("\n");
  return text.trim();
}
...
function resumeToText(resume) {
  ...
  if (resume.skills && resume.skills.length) lines.push("\nSkills: " + resume.skills.join(", "));
  ...
  (resume.education || []).forEach((ed) => lines.push(`\n${ed.degree}, ${ed.school} ${ed.year || ""}`));
...
function GenerateSection({ profile, preferences, editSignals, readiness, onAddEditSignal, onLogApplication }) {
  ...
  const handleLog = async () => {
    await onLogApplication({ company: logCompany, role: logRole, resumeSnapshot: resumeToText(result.resume), coverLetterSnapshot: editedCoverLetter });
    setLogged(true);
  };
        ...
        <div className="cc-generate-grid" style={{ marginTop: "24px" }}>
          <div className="cc-result">
            <div className="cc-result-head">
              ...
              <button type="button" className="cc-btn cc-btn-ghost cc-btn-sm" onClick={() => copyText("resume", resumeToText(result.resume))}>
      ...
      {applications.map((app, idx) => (
        <div className="cc-card" key={app.id}>
          ...
          <div className="cc-grid-2" style={{ marginTop: "12px" }}>
            ...
            <Field label="ATS score" hint="optional, self-reported"><input className="cc-input" value={app.atsScore} onChange={(e) => updateApp(idx, { atsScore: e.target.value })} /></Field>
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await window.storage.get("career-profile", false);
    ...
    (async () => {
      try {
        const res = await window.storage.get("career-applications", false);
        if (!cancelled && res && res.value) {
    ...
    (async () => {
      try {
        const res = await window.storage.get("career-edit-signals", false);
  ...
  const saveProfile = useCallback(async () => {
    ...
    try {
      ...
      await window.storage.set("career-profile", serialized, false);
  ...
  const saveApplications = useCallback(async () => {
    ...
    try {
      ...
      await window.storage.set("career-applications", serialized, false);
  ...
  const resetProfile = async () => {
    ...
    try { await window.storage.delete("career-profile", false); } catch (e) { /* ignore */ }
  ...
  const addEditSignal = useCallback(async (original, edited) => {
    ...
    setEditSignals(next);
    try { await window.storage.set("career-edit-signals", JSON.stringify(next), false); } catch (e) { /* best effort */ }
  }, [editSignals]);

  const logApplication = useCallback(async ({ company, role, resumeSnapshot, coverLetterSnapshot }) => {
    const record = {
      id: makeId(), company, role,
      dateApplied: new Date().toISOString().slice(0, 10),
      resumeSnapshot, coverLetterSnapshot,
      atsScore: "", interviewStatus: "not started", offerStatus: "n/a", notes: "",
    };
    const next = [record, ...applications];
    setApplications(next);
    try {
      await window.storage.set("career-applications", JSON.stringify(next), false);
